...
func toExternalProvider(p cp.Provider) (*ExternalProvider, error) {
	...
	case *provider.ProxySetProvider:
		...
		providerJSON, _ := json.Marshal(psp)
		providerAPI := struct {
			SubscriptionInfo *provider.SubscriptionInfo `json:"subscriptionInfo"`
		}{}
		_ = json.Unmarshal(providerJSON, &providerAPI)
		return &ExternalProvider{
			...
			SubscriptionInfo: providerAPI.SubscriptionInfo,
		}, nil
...
func overwriteConfig(targetConfig *config.RawConfig, patchConfig config.RawConfig) {
	...
	// The UI historically persisted only keep-alive-interval.  Leaving the
	// idle threshold at the platform default (often tens of minutes) lets a
	// half-closed XHTTP/H2 tunnel look healthy while an app is reusing it.  Use
	// a bounded default for the Harmony runtime, while still honoring explicit
	// values from the user's overwrite settings.
	targetConfig.KeepAliveIdle = patchConfig.KeepAliveIdle
	if targetConfig.KeepAliveIdle <= 0 {
		targetConfig.KeepAliveIdle = 15
	}
	targetConfig.KeepAliveInterval = patchConfig.KeepAliveInterval
	if targetConfig.KeepAliveInterval <= 0 {
		targetConfig.KeepAliveInterval = targetConfig.KeepAliveIdle
	}
...
func updateListeners(force bool) {
	...
	// HarmonyOS owns the TUN file descriptor through VpnExtensionAbility.
	// ReCreateTun would try to create a second platform TUN and must stay skipped.
}
...
func applyConfig(rawConfig *config.RawConfig) error {
	...
	// Keep headroom for Harmony's Android compatibility encapsulation. The
	// 1280 baseline still produced 20-60 second TCP retransmission gaps on long
	// HTTPS/WebSocket streams, so use 1200 for this A/B build.
	currentConfig.General.Tun.MTU = 1280
